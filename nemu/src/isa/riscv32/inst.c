...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, IFDEF(CONFIG_MTRACE, printf("(nemu) lbu: addr = 0x%08x, data = 0x%08x, size = %d\n", src1 + imm, (uint32_t)Mr(src1 + imm, 1), 1)); R(rd) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, IFDEF(CONFIG_MTRACE, printf("(nemu) lhu: addr = 0x%08x, data = 0x%08x, size = %d\n", src1 + imm, (uint32_t)Mr(src1 + imm, 2), 2)); R(rd) = Mr(src1 + imm, 2)); 
  INSTPAT("??????? ????? ????? 110 ????? 00000 11", lwu    , I, IFDEF(CONFIG_MTRACE, printf("(nemu) lwu: addr = 0x%08x, data = 0x%08x, size = %d\n", src1 + imm, (uint32_t)Mr(src1 + imm, 4), 4)); R(rd) = Mr(src1 + imm, 4)); 
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal		 , J, s->dnpc = s->pc + imm; IFDEF(CONFIG_FTRACE, jal_print(rd, s->pc, s->dnpc)); R(rd) = s->snpc); 
	...
	INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw	 , I, uint32_t t = C(csr); C(csr) = src1; R(rd) = t);
	INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs	 , I, uint32_t t = C(csr); C(csr) = t | src1; R(rd) = t);
	INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc	 , I, uint32_t t = C(csr); C(csr) = t & (~src1); R(rd) = t);
	INSTPAT("??????? ????? ????? 101 ????? 11100 11", csrrwi , I, R(rd) = C(csr); C(csr) = zimm);
	INSTPAT("??????? ????? ????? 110 ????? 11100 11", csrrsi , I, uint32_t t = C(csr); C(csr) = t | zimm; R(rd) = t);
	INSTPAT("??????? ????? ????? 111 ????? 11100 11", csrrci , I, uint32_t t = C(csr); C(csr) = t & (~zimm); R(rd) = t);
