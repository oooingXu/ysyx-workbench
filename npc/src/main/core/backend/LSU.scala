...
class ysyx_23060336_LSU extends Module{
  ...
  wstrb    := io.exu_lsu_data.bits.idu_lsu_data.wstrb
  MemtoReg := io.exu_lsu_data.bits.idu_lsu_data.MemtoReg
  MemWr    := io.exu_lsu_data.bits.idu_lsu_data.MemWr

  // rdata
  rdata_h := Mux(result(1,0) === 2.U, io.axi.rdata >> 16,
             Mux(result(1,0) === 0.U, io.axi.rdata, 0.U))

  rdata_b := Mux(result(1,0) === 3.U, io.axi.rdata >> 24,
             Mux(result(1,0) === 2.U, io.axi.rdata >> 16,
             Mux(result(1,0) === 1.U, io.axi.rdata >> 8, io.axi.rdata)))

  // wdata
  wdata_h := Mux(result(1,0) === 2.U, src2 << 16, 
             Mux(result(1,0) === 0.U, src2, 0.U))

  wdata_b := Mux(result(1,0) === "b11".U, Cat(src2(7,0), Fill(24, 0.U)),
             Mux(result(1,0) === "b10".U, Cat(Fill(8, 0.U),  src2(7,0), Fill(16, 0.U)),
             Mux(result(1,0) === "b01".U, Cat(Fill(16, 0.U), src2(7,0), Fill(8, 0.U)), Cat(Fill(24, 0.U), src2(7,0)))))
  ...
  DataOut := Mux(MemtoReg, Mux(io.exu_lsu_data.bits.idu_lsu_data.arsize === 0.U, rdata_b, Mux(io.exu_lsu_data.bits.idu_lsu_data.arsize === 1.U, rdata_h, io.axi.rdata)), result)
  ...
  io.lsu_wbu_data.bits.regdata := rdata

  rdata := Mux(io.exu_lsu_data.bits.idu_lsu_data.RegNum === "b010".U, DataOut,
