...
// use xiangshan ArrayMulDataModule
class ArrayMulDataModule(len: Int) extends Module {
  ...
  val s_idle :: s_wait :: s_mul :: Nil = Enum(3)
  val state = RegInit(s_idle)
  state := MuxLookup(state, s_idle)(List(
    s_idle -> Mux(io.mul_valid, s_wait, s_idle),
    s_wait -> Mux(counter === 1.U, s_mul, s_wait),
    s_mul  -> Mux(io.ready, s_idle, s_mul)
  ))

  counter := Mux(state === s_wait, counter + 1.U, 0.U)
  io.out_valid := state === s_mul
  ...
  def addAll(cols: Seq[Seq[Bool]], depth: Int): (UInt, UInt) = {
    if(max(cols.map(_.size)) <= 2){
      ...
      val toNextLayer = if(needReg)
        columns_next.map(_.map(x => RegEnable(x, io.mul_valid)))
      else
        columns_next
  ...
  val columns_reg = columns.map(col => col.map(b => RegEnable(b, io.mul_valid)))
  val (sum, carry) = addAll(cols = columns_reg.toSeq, depth = 0)

  val result = RegInit(0.U((2*len).W))
  result := sum + carry
  io.result := result
